...
if uploaded_file is not None:
    ...
    st.write("The size of the dataset is " + str(len(df)))
    number = st.number_input('How many value you want to see : ')
    number = int(number)
    endure_value = st.number_input('Enter the Endure Value')

if uploaded_file is not None and endure_value != 0 and number != 0:
    ...
    TestDataSet = NewDataSet.tail(number)
    ...
    Machine_Df['percentage of Loss'] = abs(
        (Machine_Df["TrueMegaWatt"] - Machine_Df["PredictedMegaWatt"]) / (Machine_Df["TrueMegaWatt"])) * 100

    status = []
    for v in Machine_Df['percentage of Loss']:
        if v > endure_value:
            status.append("True")
        else:
            status.append("False")

    Machine_Df["Status of thievery"] = status
